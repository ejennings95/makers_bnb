class PropertyOwner
  ...
  def self.add(name:, username:, email:, password:)
    result = Database.query( "INSERT INTO property_owner(name, username, email, password) VALUES('#{name}', '#{username}', '#{email}', '#{password}') RETURNING id, name, username, email, password;")

...
def self.add(name:, description:, price:, location:, property_owner_id:)
  result = Database.query( "INSERT INTO properties(name, description, price, location, property_owner_id) VALUES('#{name}','#{description}', '#{price}', '#{location}', '#{property_owner_id}') RETURNING name, description, price, location, property_owner_id, dates_booked;")

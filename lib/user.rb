...
class User
  ...
  def self.add(name:, username:, email:, password:)
    if (Database.query("SELECT * FROM users WHERE email = '#{email}'") || Database.query("SELECT * FROM users WHERE username = '#{username}'")).any?
      return
    else
      encrypted_password = BCrypt::Password.create(password)
      result = Database.query( "INSERT INTO users(name, username, email, password) VALUES('#{name}', '#{username}', '#{email}', '#{encrypted_password}') RETURNING id, name, username, email, password;")
